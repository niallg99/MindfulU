...
class UserProfile(models.Model):
		...
		def set_show_mood(self, show_mood):
				self.show_mood = show_mood
				self.save(update_fields=['show_mood'])

		def save(self, *args, **kwargs):
				if self.picture:
						picture_url = self.picture
						if hasattr(picture_url, 'url'):
								if not picture_url.url.startswith('https://'):
										self.picture = 'https://res.cloudinary.com/' + picture_url.url
						elif isinstance(picture_url, str) and not picture_url.startswith('https://'):
								self.picture = 'https://res.cloudinary.com/' + picture_url
